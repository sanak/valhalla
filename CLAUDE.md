...
| Adding a test fixture built by a CLI tool or script | `test/CMakeLists.txt` — pass config through `valhalla_test_config()` and `--config`, never `--inline-config` (cmd.exe does not strip the quotes), and run the extension-less python tools (`valhalla_build_extract`, …) as `${Python_EXECUTABLE} ${CMAKE_BINARY_DIR}/<tool>`. Both only fail on the windows CI |
